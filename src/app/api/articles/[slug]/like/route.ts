...
export async function GET(
  ...
  try {
    ...
    const likeToggle = await user.toggleArticleLike(article._id);
    const isLiked = user.hasLikedArticle(article._id);
    const updatedArticleLikes = isLiked
    ...
    return NextResponse.json({
      ...
      data: {
        isLiked,
        likes: updatedArticleLikes,
      },
    });
  } catch (error) {
    console.log(error, "error from like route");
    return NextResponse.json({
      success: false,
